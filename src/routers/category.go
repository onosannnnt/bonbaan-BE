...
func InitCategoryRouter(app *fiber.App, db *gorm.DB) {
	...
	service_category.Post("/addservice", handler.AddServiceToCategory) // add service to category
	service_category.Delete("/deleteservice", handler.RemoveServiceFromCategory) // remove service from category
