...
// MockUserUsecase is the mock implementation of the userUsecase.UserUsecase interface.
type MockUserUsecase struct {
	mock.Mock
}

// Update the Register method to accept *model.CreateUserRequest
func (m *MockUserUsecase) Register(user *model.CreateUserRequest) error {
	args := m.Called(user)
	return args.Error(0)
}
...
func (m *MockUserUsecase) GetByID(userID *string) (*Entities.User, error) {
    args := m.Called(userID)
    return args.Get(0).(*Entities.User), args.Error(1)
}
...
// Fixed GetByEmailOrUsername to match the UserUsecase interface.
func (m *MockUserUsecase) GetByEmailOrUsername(user *Entities.User) (*Entities.User, error) {
	args := m.Called(user)
	result, _ := args.Get(0).(*Entities.User)
	return result, args.Error(1)
}

func (m *MockUserUsecase) InsertOTP(user *Entities.User) error {
    args := m.Called(user)
    return args.Error(0)
}

func (m *MockUserUsecase) SendResetPasswordMail(user *Entities.User) error {
    args := m.Called(user)
    return args.Error(0)
}

func (m *MockUserUsecase) ResetPassword(password *model.ResetPasswordRequest) (*Entities.User, error) {
...
func TestRegister(t *testing.T) {
	...
	tests := []struct {
		name           string
		inputUser      model.CreateUserRequest
		mockError      error
		expectedStatus int
	}{
		{
			name:           "Success",
			inputUser:      model.CreateUserRequest{Email: "test@example.com", Password: "password"},
			mockError:      nil,
		...
		},
		{
			...
			inputUser:      model.CreateUserRequest{Email: "", Password: ""},
			mockError:      nil,
			expectedStatus: fiber.ErrBadRequest.Code,
		},
		{
			name:           "Internal Error",
			inputUser:      model.CreateUserRequest{Email: 	"", Password: ""},
		...
		{
			name:           "Negative Phone Test ",
			inputUser:      model.CreateUserRequest{Email: "Negative@example.com", Password: "password", Phone: "-1234567890"},
			mockError:      nil,
			expectedStatus: fiber.ErrBadRequest.Code,
		},
	}
...
func TestUpdate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:     "Success",
			userID:   validUUID,
			inputUser: model.UpdateRequest{ID: validUUID, Email: "updated@example.com"},
			mockUser: &Entities.User{
